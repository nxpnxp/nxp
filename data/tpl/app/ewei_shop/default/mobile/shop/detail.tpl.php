...
<script id='goods_info'  type='text/html'>
    ...
    <div class="good_info1">
      ...
      <div class="other">
            <div class='left'>
                <%if saleset.enoughorder<=0%>
                 ...
                 <%else%>  
                    <span><i class='fa fa-coffee'></i> 单笔订单满 <span style='color:#ff6600'><%saleset.enoughorder%></span> 元包邮</span>
                  <%/if%>
            </div>
       </div>
        <%/if%> 
        
          <%if saleset && saleset.enoughmoney>0 && saleset.enoughdeduct >0 %>
      <div class="other">
            <div class='left'>
                <span><i class='fa fa-birthday-cake'></i> 单笔订单满 <span style='color:#ff6600'><%saleset.enoughmoney%></span> 元立减 <span style='color:#ff6600'><%saleset.enoughdeduct%></span> 元</span>
            </div>
       </div>
        <%/if%> 
      ...
      <div class="other">
            <div class='left'>
                    <span><i class='fa fa-cube'></i> 可使用积分抵扣 <span style='color:#ff6600'><%goods.deduct%></span> 元</span>
            </div>
       </div>
        <%/if%> 
      ...
      <%if goods.isnodiscount=='0' && level.discount>0 && level.discount<10%>
         <div class="other">
            <div class='left'>
                <i class='fa fa-user'></i> 您是<%level.levelname%>, 可享 <span style='color:#ff6600'><%level.discount%></span> 折优惠
            </div>
         </div> 
        <%/if%>  </div>
      </div>
   ...
   <%if goods.canbuy && goods.total!=0 && goods.timebuy=='0' && goods.userbuy=='1'  && goods.groupbuy=='1'  && goods.levelbuy=='1'%> 
    ...
    <div class="good_info2">
     ...
     <?php  $shopset = m('common')->getSysset('shop')?>
    <div class="good_copyright">© 版权所有 <?php  if(!empty($shopset['name'])) { ?><?php  echo $shopset['name'];?><?php  } else { ?><?php  echo $_W['account']['name'];?><?php  } ?></div>
	...
	function choose(act){
 		 ...
 		 if( $(this).find('.on').length<=0){
	       ...
	       $('.good_choose').fadeIn(200);
                ...
                require(['core'],function(core){
					core.json('order/confirm',{op:'checkcanbuy'},function(ret){
						
						var ccate = "<?php  echo $goods['ccate'];?>";
						if(ccate == 2){
							if(ret.status==1){
