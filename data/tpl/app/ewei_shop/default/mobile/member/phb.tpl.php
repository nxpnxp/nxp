...
<head>
	...
	<link rel="stylesheet" type="text/css" href="/addons/wwx_fxxt/recouse/css/bottom.css?x=<?php echo WWX_FXXT_VERSION;?>" />
...
<div class="list-myorder" style="background:#ffffff;margin-top:-15px;">
	<ul class="ul-product" style="color:#ffcc00;font-size:20px;">
		...
		<?php  if(is_array($list)) { foreach($list as $member) { ?>
			<li>
            <span style="float:left;margin-right:10px;border-radius:3px;"><?php  echo $key;?>.<?php  if($key==1||$key==2||$key==3) { ?><?php  } ?></span>
            <span style="float:right;margin-left:10px;border-radius:3px;"><img style="width:30px;height:42px;" src="/addons/wwx_fxxt/recouse/images/000<?php  echo $key;?>.jpg" style="border-width:0px;"></span>
				<span class="pic" onClick="newMsg('<?php  echo $member["from_user"];?>');"><img src="<?php  echo tomedia($member['avatar']);?>" onerror="this.src='/addons/ewei_shop/static/images/notoo.png'"></span>
				<div class="text">
