<?php defined('IN_IA') or exit('Access Denied');?><ul class="nav nav-tabs">
    ...
    <?php if(cv('perm.log')) { ?><li <?php  if($_GPC['method']=='log') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createPluginWebUrl('perm/log')?>">操作日志</a></li><?php  } ?>
    
    <?php  if($_W['isfounder']) { ?>
    <li  <?php  if($_GPC['method']=='setting') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createPluginWebUrl('perm/setting')?>">插件设置</a></li>
   <li  <?php  if($_GPC['method']=='plugins') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createPluginWebUrl('perm/plugins')?>">插件分权</a></li>
