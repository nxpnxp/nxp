...
<div class="main">
    <form class="form-horizontal form" action="" method="post" enctype="multipart/form-data" onsubmit="return formcheck(this)">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        <p class="form-control-static"><?php  if(empty($item['addressid'])) { ?>
                            ...
                            <?php  } else { ?><?php  if(empty($dispatch)) { ?>
                             快递
                            <?php  } else { ?>
        ...
        <?php  } else if($item['isverify']==1 || !empty($item['virtual'])) { ?>
        <div class="panel panel-default">
            <div class="panel-heading">
               联系人
            </div>
                    ...
                    <tr>
                        <td colspan="3">
                            <form class="form-horizontal form" action="<?php  echo $this->createWebUrl('order')?>" method="post" enctype="multipart/form-data">
								<select class="form-control" name="print_modle_id" id="print_modle_id">
								<?php  if(is_array($express_order_list)) { foreach($express_order_list as $normal) { ?>
									<option value="<?php  echo $normal['id'];?>"><?php  echo $normal['name'];?></option>
								<?php  } } ?>
                                </select>
								...
								<button type="submit" class="btn btn-primary" name="express_print" value="yes">打印快递单</button>								
							</form>
                        </td>
						<td colspan="2">
						...
						<td colspan="3">
                            <form class="form-horizontal form" action="<?php  echo $this->createWebUrl('order')?>" method="post" enctype="multipart/form-data">
								<select class="form-control" name="print_modle_id" id="print_modle_id">
								<?php  if(is_array($normal_order_list)) { foreach($normal_order_list as $normal) { ?>
									<option value="<?php  echo $normal['id'];?>"><?php  echo $normal['name'];?></option>
								<?php  } } ?>
                                </select>
                                <input type='hidden' name='expresscom' id='expresscom' />							
								...
								<input type="hidden" name="id" value="<?php  echo $item['id'];?>" />								
								<button type="submit" class="btn btn-primary" name="express_print" value="yes">打印发货单</button>
							</form>
                        </td>
						<td colspan="2">
