...
<ul class="nav nav-tabs">
  <li <?php  if($op == 'post') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('renrentop', array('op' => 'post'));?>">添加特价</a></li>
  <li <?php  if($op == 'display') { ?>class="active"<?php  } ?>><a href="<?php  echo $this->createWebUrl('renrentop',array('op'=>'display'));?>">管理特价</a></li>
 
</ul>
<?php  if($op =='display') { ?>
<div class="panel panel-success">
  <div class="panel-heading">特价商品 > 商品管理</div>
...
<div class="panel panel-info">
		...
		<div class="panel-body">
			...
			<input type="hidden" value="renrentop" name="do">
		...
		<table class="table table-hover">
			<thead class="navbar-inner">
				...
				<th style="width:20px;">全选</th>
					...
					<th style="width:50px;">特价</th>
					<th style="min-width:60px;width:100px;">审核 | 修改 |  删除</th>
				</tr>
			</thead>
			<tbody>
				<?php  if(is_array($top)) { foreach($top as $item) { ?>
				<tr>
				<td><input type="checkbox" value="<?php  echo $item['id'];?>" name="delete[]"></td>
					...
					<td><?php  echo $item['sale'];?></a></td>
					<td>
					
<a id="id<?php  echo $item['id'];?>" href="javascript:shen('<?php  echo $this->createWebUrl('renrentop', array('op' => 'shenhe', 'id' =>$item['id'],'isok'=>$item['isok']))?>',<?php  echo $item['id'];?>);" title="<?php  if($item['isok']==0) { ?>未审核<?php  } else { ?>已审核<?php  } ?>" class="shen btn btn-mini <?php  if($item['isok']==0) { ?>btn-danger<?php  } else { ?>btn-success<?php  } ?>"><i class="fa fa-check"></i></a>
					<a href="<?php  echo $this->createWebUrl('renrentop', array('op' => 'post', 'id' => $item['id']))?>" title="编辑" class="btn btn-mini btn-primary"><i class="fa fa-edit"></i></a>
					<a onclick="return confirm('此操作不可恢复，确认吗？'); return false;" href="<?php  echo $this->createWebUrl('renrentop', array('id' => $item['id'],'op'=>'del'))?>" title="删除" class="btn btn-mini btn-danger"><i class="fa fa-times"></i></a>
					</td>				
				</tr>
...
<div class="main">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<div class="form-group">
				<label class="col-xs-12 col-sm-2 col-md-2 col-lg-1 control-label">商品特价</label>
				<div class="col-sm-8">
					<input type="text" name="sale" class="form-control" value="<?php  echo $item['sale'];?>" /> 
					<span class="help-block">填写商品特价</span>
				</div>
			</div>
		<div class="form-group">
