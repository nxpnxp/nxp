			...
			<div class="panel-body">
				...
				<?php  if(file_exists(IA_ROOT . '/attachment/qrcode_'.$account['acid'].'.jpg')) { ?>
					<div class="form-group">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								<img src="<?php  echo $_W['attachurl_local'];?>/qrcode_<?php  echo $account['acid'];?>.jpg?time=<?php  echo time()?>" class="img-responsive img-thumbnail" width="150" />
							</div>
						</div>
					</div>
				<?php  } ?>
				...
				<?php  if(file_exists(IA_ROOT . '/attachment/headimg_'.$account['acid'].'.jpg')) { ?>
					<div class="form-group">
						...
						<div class="col-sm-9 col-xs-12">
							<div class="input-group">
								<img src="<?php  echo $_W['attachurl_local'];?>/headimg_<?php  echo $account['acid'];?>.jpg?time=<?php  echo time()?>" class="img-responsive img-thumbnail" width="150" />
							</div>
						</div>
					</div>
				<?php  } ?>
				...
				<div class="form-group">
					...
					<div class="col-sm-9 col-xs-12">
						<select name="groupid" class="form-control" id="groupid">
							<option value="0">使用用户所在的用户组</option>
							<?php  if(is_array($groups)) { foreach($groups as $row) { ?>
							<option value="<?php  echo $row['id'];?>" <?php  if($row['id'] == $owner['groupid']) { ?>selected<?php  } ?>><?php  echo $row['name'];?></option>
							<?php  } } ?>
						</select>
			...
			<div class="panel-body table-responsive">
				<table class="table table-hover">
					...
					<tbody id="account-package">
						<?php  if(!empty($owner['package'])) { ?>
						<?php  if(is_array($owner['package'])) { foreach($owner['package'] as $package) { ?>
						<tr>
						...
						<tr>
							<td></td>
							<td style="line-height:25px; white-space:normal;">
								<span class="label label-success">所有模块</span>
							</td>
							<td style="line-height:25px; white-space:normal;">
								<span class="label label-success">所有模板</span>
							</td>
						</tr>
						<?php  } ?>
					</tbody>
	...
	<!--
		$(function(){
			...
			require(['biz', 'underscore'], function(biz, _){
				var seletedUserIds = [];
				var groupitem = '<%_.each(package, function(item) {%> \n' +
								'<tr>'+
								'	<td><%=item.name%></td>'+
								'	<td style="line-height:25px; white-space:normal;">'+
								'		<span class="label label-success">系统模块</span>'+
								...
								'	<td style="line-height:25px; white-space:normal;">'+
								'		<span class="label label-success">微站默认模板</span>'+
								'		<%_.each(item.templates, function(template) {%> \n' +
				...
				$('#btn-add').click(function(){
					...
					biz.user.browser(seletedUserIds, function(user){
						$.getJSON('./index.php?c=account&a=post-step&do=userinfo&step=3&uid='+user, function(data){
							...
							$('#account-package').html(_.template(groupitem)(data.message));
				...
				$('#groupid').change(function(){
					...
					$.getJSON('./index.php?c=account&a=post-step&do=userinfo&step=3&uid='+user+'&groupid='+$('#groupid').val(), function(data){
						if (data.message.errno) {
							util.message(data.message.message);
						}
						$('#account-package').html(_.template(groupitem)(data.message));
