...
if ($_W['isajax']) {
    if ($operation == 'display') {
        ...
        $needpay = false;
        if ($goods['money'] > 0) {
            pdo_delete('ewei_shop_creditshop_log', array(
                'goodsid' => $id,
                'openid' => $openid,
                'status' => 0,
                'paystatus' => 0
            ...
            $needpay = true;
            $lastlog = pdo_fetch('select * from ' . tablename('ewei_shop_creditshop_log') . ' where goodsid=:goodsid and openid=:openid  and status=0 and paystatus=1 and uniacid=:uniacid limit 1', array(
                ':goodsid' => $id,
                ':openid' => $openid,
                ':uniacid' => $uniacid
            ));
        ...
        $log = array(
            ...
            'dispatchstatus' => $goods['isverify'] == 1 ? -1 : 0,
            'createtime' => time()
        );
        if (empty($goods['type'])) {
        ...
        pdo_update('ewei_shop_creditshop_log', $update, array(
            'id' => $logid
        ));
        if ($status == 2) {
...
$_W['shopshare'] = array(
    ...
    'link' => $this->createPluginMobileUrl('creditshop/detail', array(
        'id' => $id
    )),
