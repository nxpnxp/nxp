...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('ewei_shop_creditshop_goods', array(
        ...
        message('商品排序更新成功！', $this->createPluginWebUrl('creditshop/goods', array(
            'op' => 'display'
        )), 'success');
    }
    ...
    $category = pdo_fetchall("select id,name,thumb from " . tablename('ewei_shop_creditshop_category') . ' where uniacid=:uniacid order by displayorder desc', array(
        ':uniacid' => $_W['uniacid']
    ), 'id');
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_creditshop_goods', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
        ...
        message('更新商品成功！', $this->createPluginWebUrl('creditshop/goods', array(
            'op' => 'post',
            'id' => $id
        )), 'success');
    }
    $item = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_creditshop_goods') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    if ($item['showlevels'] != '') {
    ...
    $id   = intval($_GPC['id']);
    $item = pdo_fetch("SELECT id,title FROM " . tablename('ewei_shop_creditshop_goods') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    if (empty($item)) {
        message('抱歉，商品不存在或是已经被删除！', $this->createPluginWebUrl('creditshop/goods', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_update('ewei_shop_creditshop_goods', array(
    ...
    message('商品删除成功！', $this->createPluginWebUrl('verify/saler', array(
        'op' => 'display'
    )), 'success');
