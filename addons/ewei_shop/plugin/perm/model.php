...
if (!class_exists('PermModel')) {
    class PermModel extends PluginModel
    {
                ...
                'statistics' => array(
                    ...
                    'child' => array(
                        ...
                        'export' => array(
                            ...
                            'order' => '导出订单统计-log',
                            'goods' => '导出商品销售统计-log',
                            'goods_rank' => '导出商品销售排行-log',
                            'goods_trans' => '商品销售转化率-log',
                            'member_cost' => '会员消费排行-log'
                        )
                    )
                ),
                'sysset' => array(
        ...
        private function check($permtype = '')
        {
            ...
            $user = pdo_fetch('select u.status as userstatus,r.status as rolestatus,u.perms as userperms,r.perms as roleperms from ' . tablename('ewei_shop_perm_user') . ' u ' . ' left join ' . tablename('ewei_shop_perm_role') . ' r on u.roleid = r.id ' . ' where uid=:uid limit 1 ', array(
                ':uid' => $uid
            ));
        ...
        function check_plugin($pluginname = '')
        {
            ...
            $type = intval(@file_get_contents(IA_ROOT . "/addons/ewei_shop/data/perm/set"));
            if (!empty($type)) {
                $acid    = pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE `uniacid`=:uniacid LIMIT 1", array(
                    ':uniacid' => $_W['uniacid']
                ));
                $ac_perm = pdo_fetch('select  plugins from ' . tablename('ewei_shop_perm_plugin') . ' where acid=:acid limit 1', array(
                    ':acid' => $acid
                ));
                if (!empty($ac_perm)) {
                    ...
                    $user_perm = pdo_fetch('select  plugins from ' . tablename('ewei_shop_perm_plugin') . ' where uid=:uid limit 1', array(
                        ':uid' => $_W['uid']
                    ));
                    if (!empty($user_perm)) {
                        $allow_plugins = explode(',', $user_perm['plugins']);
                        if (!in_array($pluginname, $allow_plugins)) {
                            $allow = false;
                        }
                    } else {
                    ...
                    foreach ($p['child'] as $ck => $child) {
                        foreach ($child as $k => $v) {
                            if (strexists($v, '-log')) {
                                ...
                                $types[] = array(
                                    'text' => $text,
                                    'value' => str_replace(".text", "", $pk . "." . $ck . "." . $k)
                                );
                            }
                        }
                    }
