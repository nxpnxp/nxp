...
if (checksubmit('submit_admin')) {
	...
	load()->func('file');
	@mkdirs(IA_ROOT . '/addons/ewei_shop/data/sysset', '0777');
	file_put_contents(IA_ROOT . '/addons/ewei_shop/data/sysset/qiniu', json_encode($set['admin']));
	plog('qiniu.admin', '设置七牛');
	message('设置保存成功!', referer(), 'success');
}
$set['admin'] = @json_decode(@file_get_contents(IA_ROOT . "/addons/ewei_shop/data/sysset/qiniu"), true);
