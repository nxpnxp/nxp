...
if ($operation == 'display') {
    ...
    $list = pdo_fetchall("SELECT s.*,m.nickname,m.avatar,m.mobile,m.realname,store.storename FROM " . tablename('ewei_shop_saler') . "  s " . " left join " . tablename('ewei_shop_member') . " m on s.openid=m.openid " . " left join " . tablename('ewei_shop_store') . " store on store.id=s.storeid " . " WHERE s.uniacid = '{$_W['uniacid']}' ORDER BY id asc");
    ...
    $item = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_saler') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':uniacid' => $_W['uniacid'],
        ':id' => $id
    ));
    if (!empty($item)) {
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_saler', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
    ...
    pdo_delete('ewei_shop_saler', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
