...
class PluginModel
{
    ...
    public function updateSet($data = array())
    {
        ...
        $set     = m('common')->getSetData();
        if (empty($set)) {
            pdo_insert('ewei_shop_sysset', array(
                'uniacid' => $uniacid,
            ...
            pdo_update('ewei_shop_sysset', array(
    ...
    function getName()
    {
        return pdo_fetchcolumn('select name from ' . tablename('ewei_shop_plugin') . ' where identity=:identity limit 1', array(
