...
if ($operation == 'display') {
    ...
    foreach ($list as &$value) {
        ...
        if (!empty($agentid) && $level > 0) {
            ...
            if ($value['agentid'] == $agentid) {
                $value['level']     = 1;
                $level1_commissions = pdo_fetchall('select commission1  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and o.agentid= ' . $agentid . "  and o.uniacid=:uniacid", array(
                    ':orderid' => $value['id'],
                    ':uniacid' => $_W['uniacid']
                ));
                ...
                $value['level'] = 2;
                if ($agent['level2'] > 0) {
                    $level2_commissions = pdo_fetchall('select commission2  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',', array_keys($agent['level1_agentids'])) . ")  and o.uniacid=:uniacid", array(
                        ':orderid' => $value['id'],
                        ':uniacid' => $_W['uniacid']
                    ));
                ...
                $value['level'] = 3;
                if ($agent['level3'] > 0) {
                    $level3_commissions = pdo_fetchall('select commission3 from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',', array_keys($agent['level2_agentids'])) . ")  and o.uniacid=:uniacid", array(
                        ':orderid' => $value['id'],
                        ':uniacid' => $_W['uniacid']
                    ));
    ...
    exit;
} elseif ($operation == 'express_print') {
			$id = intval($_GPC['id']);
			...
			//print_r('<pre>');print_r($address);exit;
			$pid = intval($_GPC['print_modle_id']);
			$entry = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_printer') . ' WHERE  id=:id', array(':id' => $pid));
			//print_r('<pre>');print_r($entry);exit;
			$buyer = '';
			//$member   = m('member')->getInfo($item['openid']);
			$wwx_fxxt_member = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_member') . ' WHERE  uniacid=:uniacid and openid=:from_user', array(':uniacid' => $_W['uniacid'], ':from_user' => $item['openid']));
			//print_r('<pre>');print_r($wwx_fxxt_member);exit;
			...
			for ($i = 1; $i < 3; $i++) {
				...
				$goods = pdo_fetchall('SELECT g.*,o.price  as \'order_price\' ,o.total as \'order_total\',o.optionname as \'order_optionname\' FROM ' . tablename('ewei_shop_order_goods') . ' o left join ' . tablename('ewei_shop_goods') . ' g on o.goodsid=g.id ' . " WHERE o.orderid='{$id}'");
				$goodsmoney = 0;
...
function order_list_refund($item)
{
    ...
    $refund = pdo_fetch('select * from ' . tablename('ewei_shop_order_refund') . ' where id=:id and status=0 limit 1', array(
        ':id' => $item['refundid']
    ));
    if (empty($refund)) {
        pdo_update('ewei_shop_order', array(
    ...
    if ($refundstatus == 0) {
        ...
        $goods     = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename('ewei_shop_order_goods') . " o left join " . tablename('ewei_shop_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(
            ':orderid' => $item['id'],
            ':uniacid' => $_W['uniacid']
        ));
        $credits   = 0;
