...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['datas'])) {
        ...
        foreach ($datas as $row) {
            pdo_update("ewei_shop_category", array(
                'parentid' => 0,
            ...
            if ($row['children'] && is_array($row['children'])) {
                ...
                foreach ($row['children'] as $child) {
                    pdo_query("update " . tablename('ewei_shop_category') . " set  parentid=:parentid,displayorder=:displayorder,level=2 where id=:id", array(
                    ...
                    if ($child['children'] && is_array($child['children'])) {
                        ...
                        foreach ($child['children'] as $third) {
                            pdo_query("update " . tablename('ewei_shop_category') . " set  parentid=:parentid,displayorder=:displayorder,level=3 where id=:id", array(
	...
	if (!empty($item['id']) && ($item['id']==1 || $item['id']==2 || $item['id']==5) ) {
        $cat_com     = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_category_commission') . " WHERE cateid = ".$item['id']." limit 1");
    }
    ...
    if (checksubmit('submit')) {
        ...
        $data1 = array(
            ...
            'f2c_max' => $_GPC['f2c_max']
        );
        if (!empty($id)) {
            unset($data['parentid']);
            pdo_update('ewei_shop_category', $data, array(
                'id' => $id
            ));
            pdo_update('ewei_shop_category_commission', $data1, array(
                'cateid' => $id
            ));
            load()->func('file');
    ...
    pdo_delete('ewei_shop_category', array(
        'id' => $id,
        'parentid' => $id
    ), 'OR');
