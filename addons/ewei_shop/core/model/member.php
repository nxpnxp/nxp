...
class Ewei_DShop_Member
{
    ...
    function upgradeLevel($openid)
    {
        ...
        $member = m('member')->getMember($openid);
        if (!empty($member)) {
            $ordercount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order') . ' where openid=:openid and status=3 and uniacid=:uniacid ', array(
                ':uniacid' => $_W['uniacid'],
                ':openid' => $member['openid']
            ));
            $ordermoney = pdo_fetchcolumn('select ifnull( sum(price),0) from ' . tablename('ewei_shop_order') . ' where openid=:openid and status=3 and uniacid=:uniacid ', array(
                ':uniacid' => $_W['uniacid'],
                ':openid' => $member['openid']
            ));
            $level      = pdo_fetch('select * from ' . tablename('ewei_shop_member_level') . " where uniacid=:uniacid " . " and  ( ({$ordercount} >= ordercount and ordercount>0) or  ({$ordermoney} >= ordermoney and ordermoney>0) ) " . " order by level desc limit 1", array(
                ':uniacid' => $_W['uniacid']
            ));
	...
	function updateonetwo($orderid)
    {
    	//支付成功后  更新member表的topcommissionlevel字段
        global $_W;
		if (empty($orderid)) {
            return;
        }
		$orderinfo = pdo_fetch('select openid from ' . tablename('ewei_shop_order') .' where id=:id and uniacid=:uniacid limit 1',array(
				':uniacid' => $_W['uniacid'],
                ':id' => $orderid
		));
        $member = m('member')->getMember($orderinfo['openid']);
        if (!empty($member)) {
            $onecount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid and cateid=:cateid', array(
                ':uniacid' => $_W['uniacid'],
                ':orderid' => $orderid,
                ':cateid' => 1
            ));
            $twocount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_order_goods') . ' where orderid=:orderid and uniacid=:uniacid and cateid=:cateid', array(
                ':uniacid' => $_W['uniacid'],
                ':orderid' => $orderid,
                ':cateid' => 2
            ));
			...
			if(!empty($onecount)){
				$data['one'] = 1;
				$data['topcommissionlevel'] = 1;
			}
			if(!empty($twocount)){
				$data['two'] = 1;
				$data['topcommissionlevel'] = 1;
			}
			if(!empty($onecount) || !empty($twocount)){	
