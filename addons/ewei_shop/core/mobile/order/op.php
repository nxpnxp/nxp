...
if ($_W['isajax']) {
    if ($operation == 'cancel') {
        ...
        $order   = pdo_fetch("select id,ordersn,openid,status,deductcredit,deductprice from " . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(
            ':id' => $orderid,
            ':uniacid' => $uniacid,
            ':openid' => $openid
        ));
        ...
        $orderid = intval($_GPC['orderid']);
        $order   = pdo_fetch("select id,status,openid from " . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(
            ':id' => $orderid,
            ':uniacid' => $uniacid,
            ':openid' => $openid
        ));
        ...
        $orderid = intval($_GPC['orderid']);
        $order   = pdo_fetch("select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,virtual from " . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(
            ':id' => $orderid,
            ':uniacid' => $uniacid,
            ':openid' => $openid
        ));
        ...
        if ($_W['ispost']) {
            if (!empty($_GPC['cancel'])) {
                ...
                $refund = array(
                    ...
                    'reason' => $_GPC['refunddata']['reason'],
                    'content' => $_GPC['refunddata']['content']
                );
                if (empty($refundid)) {
        ...
        $orderid = intval($_GPC['orderid']);
        $order   = pdo_fetch("select id,status from " . tablename('ewei_shop_order') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1', array(
            ':id' => $orderid,
            ':uniacid' => $uniacid,
            ':openid' => $openid
        ));
