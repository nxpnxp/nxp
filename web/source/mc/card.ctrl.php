...
$dos = array('display', 'manage', 'delete', 'coupon', 'submit', 'modal', 'record');
...
if ($do == 'manage') {
	...
	if(!empty($keyword)) {
		$where .= ' AND uid IN (SELECT uid FROM '. tablename('mc_members') ." WHERE uniacid = {$_W['uniacid']} AND (mobile LIKE '%{$keyword}%' OR realname LIKE '%{$keyword}%'))";
	}
	$sql = 'SELECT * FROM ' . tablename('mc_card_members') . " WHERE uniacid = :uniacid $where ORDER BY id DESC LIMIT ".($pindex - 1) * $psize.','.$psize;
	$list = pdo_fetchall($sql, $param);
	$total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('mc_card_members') . " WHERE uniacid = :uniacid $where", $param);
	...
	$members = mc_fetch($uids, array('realname', 'mobile', 'credit1', 'credit2', 'groupid'));
	foreach ($list as &$value) {
		$value['realname'] = $members[$value['uid']]['realname'];
		$value['mobile'] = $members[$value['uid']]['mobile'];
		$value['credit1'] = $members[$value['uid']]['credit1'];
		$value['credit2'] = $members[$value['uid']]['credit2'];
		$value['groupid'] = $members[$value['uid']]['groupid'];
	}
}
