...
if(!empty($xkey)) {
	if ($xkey=='we7_account') {
		$tsql = "SELECT do FROM " . tablename('modules_bindings') . " WHERE module='we_xkey' and entry='menu' and title='公众号管理'";
		$we_xkey = pdo_fetchcolumn($tsql);
		if(empty($we_xkey)) {
			$data = array(
				'module' => 'we_xkey',
				'entry' => 'menu',
		...
		pdo_delete('modules_bindings', array('module' => 'we_xkey','entry' => 'cover','title' => '公众号管理'));
	}
}
$tsql = "SELECT do FROM " . tablename('modules_bindings') . " WHERE module='we_xkey' and entry='cover' and title='公众号管理'";
$we_xkey = pdo_fetchcolumn($tsql);
if(empty($we_xkey)) {
	$we_xkey='modify';
} else {
